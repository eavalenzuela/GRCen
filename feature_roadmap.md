...
`/frameworks` lists every framework with a coverage bar; `/frameworks/{id}` shows requirements (✓ satisfied / gap with satisfiers), audits, vendors, in-scope assets, and a "Last audited" column derived from `audit_log` timestamps. Gap report exports as CSV (`/frameworks/{id}/gap-report.csv`) and PDF (`/frameworks/{id}/gap-report.pdf`). Inverted view at `/controls` lists every Control with the requirements it covers. **Borrowed coverage (comply once, satisfy many):** a direct-gap requirement that cross-maps `equivalent` to a satisfied requirement in another framework is credited as *covered via crosswalk* — a third coverage tier surfaced on the detail page, gap CSV/PDF, and `GET /api/frameworks/{id}`, with an "effective coverage %" alongside direct, a framework×framework crosswalk matrix on the index, and `GET /api/frameworks/crosswalk-matrix`. Remaining: control-library export.
