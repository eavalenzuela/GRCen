...
`/frameworks` lists every framework with a coverage bar; `/frameworks/{id}` shows requirements (✓ satisfied / gap with the satisfiers), audits, vendors, and in-scope assets. Now also includes a "Last audited" column derived from the most recent `audit_log` update timestamp on each requirement or its satisfiers. CSV export at `/frameworks/{id}/gap-report.csv` (one row per requirement, satisfiers concatenated). Inverted view at `/controls` lists every Control with the requirements it covers — handy for the "what does this control protect?" query. Remaining: per-framework PDF gap report (HTML CSV exists), per-audit reports, branded cover pages.
...
Local users enroll TOTP from `/settings`: QR code, eight single-use recovery codes (stored SHA-256 hashed). Login redirects to `/login/mfa` when MFA is enabled and accepts either the TOTP or a recovery code. Per-role enforcement via `settings.MFA_REQUIRED_FOR_ROLES` (comma-separated, e.g. `"admin"`) — users in those roles can't sign in until they've enrolled. SSO users skip the gate (their IdP enforces). Remaining: FIDO2 / WebAuthn, optional encryption of TOTP secrets at rest.

## Tier 4 — Hardening & Operational

### 17. General API Rate Limiting — **SHIPPED**
`RateLimitMiddleware` runs in front of every non-exempt request. Sliding-window-per-minute counters keyed by (caller, bucket, prefix): caller is the API token → session id → client IP, bucket is `read` vs `write`, prefix is the matching route override or `""`. Default budgets 600 read / 120 write per minute. Per-route overrides via `settings.RATE_LIMIT_ROUTE_OVERRIDES` (`"<prefix>:<read>:<write>,..."`) — longest matching prefix wins, and each override gets its own counter so a tightened endpoint doesn't share a budget with the global pool. `/health`, `/static/*`, `/login`, `/logout` exempt. 429 responses include `Retry-After` plus `X-RateLimit-Limit` / `X-RateLimit-Remaining`. Remaining: shared backend (Redis) so multi-worker deployments don't undercount, admin-config UI for the budgets.
