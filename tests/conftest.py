...
async def clean_tables(pool):
    ...
    _reset_rate_limit()
    yield
    for table in ("sessions", "api_tokens", "app_settings", "audit_log", "data_access_log", "user_totp", "webhook_deliveries", "webhooks", "notification_deliveries", "notifications", "alerts", "attachments", "relationships", "risk_snapshots", "risk_appetite", "compliance_snapshots", "control_test_runs", "saved_searches", "pending_change_comments", "pending_change_approvals", "pending_changes", "pending_email_digest", "workflow_config", "sensitive_field_overrides", "asset_sensitive_overrides", "user_organizations", "assets", "users", "encryption_config"):
