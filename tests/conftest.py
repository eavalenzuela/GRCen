...
async def clean_tables(pool):
    ...
    _reset_rate_limit()
    yield
    for table in ("sessions", "api_tokens", "app_settings", "audit_log", "webhook_deliveries", "webhooks", "notification_deliveries", "notifications", "alerts", "attachments", "relationships", "risk_snapshots", "saved_searches", "assets", "users", "encryption_config"):
